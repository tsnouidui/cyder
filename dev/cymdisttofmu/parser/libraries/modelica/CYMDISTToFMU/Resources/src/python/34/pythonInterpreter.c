...
void pythonExchangeValuesCymdistNoModelica(const char * moduleName,
						  ...
						  const char ** strLocRea, double * dblValRea, size_t nDblParWri, 
			              ...
			              void (*ModelicaFormatError)(const char *string,...))
{
  ...
  Py_ssize_t nStrTypWri = 0;
  Py_ssize_t nStrLocWri = 0;
...
The error message is \"%s\"",
  ...
  if (nDblWri > 0){
	  /* Increase the number of arguments to 4*/
	  /* One is for the vector of input names*/
	  /* to be sent to CYMDISTToFMU, the second one is*/
	  /* for the vector of input types to be sent*/
	  /* the third one is for the vector of input */
	  /* location to be sent. The fourth one is the */
	  /* vector of input values to be sent to CYMDISTFMU.*/
	  nStrWri = nDblWri;
	  nStrTypWri = nDblWri;
	  nStrLocWri = nDblWri;
	  nArg = nArg + 4;
  ...
  if (nDblRea > 0){
	  ...
	  /* output location names of the outputs*/
  ...
  if ( nStrTypWri > 0 ){
    pArgsStr = PyList_New(nStrTypWri);

    for (i = 0; i < nStrTypWri; ++i) {
      /* Convert argument to a python float*/
      /*      Py_ssize_t len = 0;*/
      /* According to the Modelica Specification, strings are terminated by '\0'*/
      /* Seek the string length*/
      ...
      /*      pValue = PyUnicode_FromStringAndSize(strWri[i], len);*/
      pValue = PyUnicode_FromString(strTypWri[i]);
      if (!pValue) {
	/* Failed to convert argument.*/
	Py_DECREF(pArgsStr);
	Py_DECREF(pModule);
  ...
  if ( nStrLocWri > 0 ){
    pArgsStr = PyList_New(nStrLocWri);
    ...
    for (i = 0; i < nStrTypWri; ++i) {
      ...
      /*      pValue = PyUnicode_FromStringAndSize(strWri[i], len);*/
      pValue = PyUnicode_FromString(strLocWri[i]);
      if (!pValue) {
	/* Failed to convert argument.*/
	Py_DECREF(pArgsStr);
	Py_DECREF(pModule);
  ...
  /* i) Convert char **, an array of character arrays*/
  if (nStrParWri > 0){
  ...
  /* j) Convert flag for writing results*/
